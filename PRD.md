...
*Corporate* is the product this is sold and written as, and since 19 Sep it is **the default**: an install with no `TOC_PROFILE` set opens on the corporate desk — S1–S3 under their staff codes, the flat team list, the executive-protection sample, posture in words rather than DEFCON (Decision AC), S4 and S6 off but available. The default is itself a positioning statement, which is why it moved (Decision AD). The corporate sample is not the thinner one: it carries more threats, PIRs, NAIs, movements and trips than the brigade does, and the brigade's advantage is headcount and the sections a corporate desk does not run.
