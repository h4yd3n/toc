...
def test_signing_in_resolves_role_and_perms(client):
    me = client.get("/v1/cop/me", headers=U("u_supply")).json()
    assert me["name"].startswith("SSG") and me["role"] == "logistics" and me["perms"] == {"S1": "view", "S3": "view", "S4": "edit"}
    assert me["sections_visible"] == ["S1", "S3", "S4"] and not me["battle_captain"] and not me["admin"]
    bc = client.get("/v1/cop/me", headers=U("u_bc")).json()
    ...
    assert client.get("/v1/cop/snapshot", headers=U("u_s2")).json()["me"]["role"] == "analyst"


def test_permissions_gate_writes(client):
    # the supply sergeant edits S4 and nothing else
    assert client.patch("/v1/cop/supply/sup_001", json={"on_hand": 1}, headers=U("u_supply")).status_code == 200
    assert client.patch("/v1/cop/systems/sys_001", json={"status": "down"}, headers=U("u_supply")).status_code == 403
    assert client.post("/v1/cop/incidents", json={"location_id": "loc_bde"}, headers=U("u_supply")).status_code == 403
    assert client.get("/v1/cop/settings", headers=U("u_supply")).status_code == 403
    # a brigade S3 with only S4 view cannot change S4; the Battle Captain can do anything
    assert client.patch("/v1/cop/supply/sup_001", json={"on_hand": 2}, headers=U("u_s3")).status_code == 403
    assert client.patch("/v1/cop/systems/sys_001", json={"status": "up"}, headers=U("u_bc")).status_code == 200
    log = client.get("/v1/cop/log?limit=1").json()[0]
    assert log["actor"] == "MAJ Dakota Vance"  # the signed-in name is the actor on the ledger


def test_admin_grants_permissions(client):
    assert client.post("/v1/cop/users", json={"name": "x"}, headers=U("u_supply")).status_code == 403
    ...
    listing = client.get("/v1/cop/users", headers=U("u_supply")).json()
...
def test_directory_follows_the_dataset(client):
    ...
    assert {"u_bc", "u_supply", "u_s6"} <= names
    client.post("/v1/cop/seed?dataset=corporate")
    names = {u["id"] for u in client.get("/v1/cop/users", headers=BC).json()["users"]}
    assert "u_ep" in names and "u_supply" not in names
