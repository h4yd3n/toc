...
def test_s1_roster_builds_the_task_organization(client):
    ...
    alex = next(p for p in snap["people"] if p["last_name"] == "Nguyen" and p["first_name"] == "Alex")
    assert alex["name"] == "NGUYEN, Alex · SSG" and alex["grade"] == "E6" and teams[alex["team_id"]]["short"] == "A/1" and alex["phone"] == "270-555-0100" and alex["source"] == "upload:roster.xlsx"
    riley = next(p for p in snap["people"] if p["last_name"] == "Brooks")
    ...
    dana = next(p for p in snap["people"] if p["last_name"] == "Kim" and p["first_name"] == "Dana")
...
def test_s4_supply_sheet_with_classes_of_supply(client):
    ...
    pv = client.post("/v1/cop/upload/S4/preview", files={"file": ("logstat.xlsx", data, "application/octet-stream")}, headers=U("u_logistics")).json()
    ...
    res = client.post("/v1/cop/upload/S4/commit", json={"upload_id": pv["upload_id"], "sheet": pv["sheet"], "mapping": pv["mapping"], "kind": "supply"}, headers=U("u_logistics")).json()
    ...
    assert client.post("/v1/cop/upload/S4/preview", files={"file": ("x.xlsx", data)}, headers=U("u_signal")).status_code == 403
...
def test_s6_and_s3_sheets_and_csv(client):
    ...
    pv = client.post("/v1/cop/upload/S6/preview", files={"file": ("comms.xlsx", data)}, headers=U("u_signal")).json()
    assert pv["mapping"]["OPSTAT"] == "status" and pv["mapping"]["PACE"] == "pace" and pv["mapping"]["CP"] == "site"
    res = client.post("/v1/cop/upload/S6/commit", json={"upload_id": pv["upload_id"], "sheet": "Comms", "mapping": pv["mapping"]}, headers=U("u_signal")).json()
    ...
    pv = client.post("/v1/cop/upload/S3/preview", files={"file": ("plan.csv", csv_text.encode())}, headers=U("u_ea")).json()
    assert pv["mapping"]["Activity"] == "name" and pv["mapping"]["Start"] == "start" and pv["mapping"]["Location"] == "place" and pv["mapping"]["Who"] == "who"
    res = client.post("/v1/cop/upload/S3/commit", json={"upload_id": pv["upload_id"], "sheet": pv["sheet"], "mapping": pv["mapping"]}, headers=U("u_ea")).json()
