    ...
    async def add():
        async with sessions()() as s:
            s.add(ThreatRow(id="thr_pt_adv", external_id="state_dept:PT", title="Portugal — Level 1: Exercise Normal Precautions", summary="No changes.", lat=38.7223, lon=-9.1393, radius_km=0,
                            severity="low", event_type="advisory", source="state_dept", confidence="high", observed_at=datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=200), synthetic=False, country="PT", scope="country"))
            await s.commit()
    asyncio.run(add())
