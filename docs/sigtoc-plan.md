...
| :--- | :--- | :--- |
| **Actor** | The other side as a thing with a name: an enemy unit with echelon and strength on a military desk; a threat actor — an individual, a group, an organization — with type and size on a corporate desk. Same object, profile-named, like ranks and the graphics catalog. Carries an order-of-battle card: composition, equipment, known TTPs, assessed intent, last known position | Red icon on the COP and the S2 overlay at its last known position; the S2 panel's *Who is out there* |
| **Sighting** | One observation of an actor: time, place, source (a report, a feed, a liaison), grade, what was seen. The chain of sightings is the track | Breadcrumbs behind the actor on S2; the time wheel; the threshold rules |
| **Report** | A SPOTREP / SALUTE / SITREP from our own people, filed from a phone or the wall, with place and time. Exists today but only inside a case; becomes first-class on the map | A pin on the picture until the analyst disposes of it: corroborate, link (to an actor, a threat, a case, an NAI), or dismiss |
| **Threat graphic** | The threat overlay drawn by hand from the graphics catalog (§3.4), extended for S2: danger area, likely ambush site, engagement / kill zone, attack or IED hot spot, avenue of approach, mobility corridor, no-go and slow-go terrain, restricted area, obstacle or UXO, hostile checkpoint, hostile observation post, surveillance detection point. Each carries confidence and a valid window as well as the source | S2 overlay; and on S3 wherever a movement leg crosses one |
| **Decision point** | The event template's join: an NAI, the indicator watched there, the PIR it serves, and the decision it informs — "if X is seen at NAI 3 before 1800, the convoy takes ASR BLUE". Drawn as a graphic, tied to a requirement and to a graphic on S3 | S2 overlay; the decision support matrix on the panel; the brief |
...
- **Actors** (`s2_actors`): id, profile-named type (unit / individual / group / organization), name and aliases,
  ...
  panel and a red marker at the last known position. On the COP the actor's marker is the enemy situation; on S2 it
  carries its track.
- **Sightings** (`s2_sightings`): actor, time, place (lat/lon, place name, and the NAI it fell in, computed), source
  ...
  (confirmed / probable / possible). The last confirmed or probable sighting is the last known position. Sightings
  older than a configurable age draw faint; the track is the last N.
...
- **SPOTREP from the phones.** A form on S1 (who saw it), S2 (what), and the COP tab: SALUTE fields — size, activity,
  ...
  photo later. Filed as a report with the reporter's identity and grade A2. Lands on the wall's map inside seconds as a
  pin with a REPORT chip on the S2 rail badge.
- **Disposition on the wall.** The analyst opens a report and does one of four things: *corroborate* (raises
  credibility, marks it on the source), *link* to an actor as a sighting, to a threat, to a case, or to an NAI, *promote*
  to a threat (draws the ring), or *dismiss* with a reason. Every disposition on the ledger. Reports with no
  disposition after a configurable time show as a backlog count on the S2 headline.
...
- **The ISR synchronization matrix** as a view, not a new object: NAIs down the side, time across, and in each cell
...
| :--- | :--- | :--- |
| **1** | LOE 1 in full; LOE 2's SPOTREP from phones and wall, and disposition | The objects everything else reads. Ends with a red force on the map, a threat overlay S3 can feel, and reports flowing in from the field |
| **2** | LOE 2's RFI, collection from the map, the ISR sync view; LOE 3's pattern of life and thresholds | Closes the cycle: ask, collect, see, be warned. Cheap once phase 1 exists |
| **3** | LOE 3's IPB products, threat COAs, decision points and the DSM; LOE 4's annex and estimate | The staff products, which need everything above to be worth generating |
...
1. Threat overlay as the graphics object extended, or a separate object. Recommendation: extended.
2. "SR zones" in the author's note — if this means surface danger zones, those belong to S3's range graphic; if it
   means something else (a specific restricted-zone type), name it and it goes into the S2 catalog.
3. Whether phones file SPOTREPs in phase 1 (recommended) or the wall alone first.
4. The threshold defaults for the first rule: three sightings, seven days, one NAI is the proposal.
