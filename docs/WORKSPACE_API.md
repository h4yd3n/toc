...
**Landed 17 Sep.** Four of these are built:

- **Typed proposals that update the case graph.** `Analysis.proposed_graph` carries entity, relationship and event proposals; each names the finding that supports it and so inherits that finding's quote. `POST /v1/work/runs/{id}/graph` applies one, and only after review, only on a case-scoped assignment, and only into the `suggested` state: the analyst still confirms or rejects it in the case's own queue. A proposal quoting a report carries that report's grade; anything else is F6. A link whose ends are not already in the case is refused rather than inventing them; an event must cite a report, so it is dated and placed by that report rather than by the run. Applying the same entity twice adds the citation to the existing node instead of creating a second one.
- **Operation- and requirement-scoped assignments.** `subject_type` / `subject_id` on an assignment (`operation` or `requirement`). An operation's scope is its own record — tasks, resources, notes and what it is about; a requirement's is the reporting and sightings inside its radius plus the question itself. A scoped assignment reads only that, not the section-wide pass.
- **Repeat suppression across assignments.** A finding another assignment in the same section has already made — the same claim once normalised, or the same quote from the same source — is marked in the run's metrics as a repeat and shown as one in the console, so it is not read as a second, independent report. Exact matching, not semantic: matching meaning would need embeddings, which this does not have, and a near-miss silently dropped would be worse than a repeat shown. **Semantic** duplicate suppression remains open.
- **Automatic case matching for reporting.** `GET /v1/s2/reports/{id}/case-matches` returns the open cases a loose report probably belongs to and *why* — the case names something the report names, the case's subject is within 5 km of where it was filed, or the same reporter has filed into it before. No score. Attaching is still the analyst's act through the attach endpoint. Requirement matching (which NAI a report answers) is derived on the wall already and is not repeated here.

Live-provider quality/cost evaluation and final runtime-model selection remain open, and cannot be closed without a funded provider account. Other remaining items: consolidated extraction/drafting adapters; shared saved filters across every view; semantic duplicate suppression; and native workspace parity (the phones open the console in-app rather than drawing it natively). Those are planned work, not implied by the existing Assign analysis button.
