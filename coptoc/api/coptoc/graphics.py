...
def catalog(profile: str) -> List[Dict[str, Any]]:
    ...
    return [{"type": t, "section": c["section"], "kinds": list(c["kinds"]), "label": c["military" if profile == "military" else "corporate"], "color": c["color"], "dash": c["dash"], "glyph": c["glyph"]} for t, c in CATALOG.items()]
...
def out(g: GraphicRow, now: datetime, profile: str) -> Dict[str, Any]:
    ...
    return {"id": g.id, "type": g.type, "kind": g.kind, "section": g.section, "name": g.name, "label": c["military" if profile == "military" else "corporate"],
            ...
            "status": g.status, "note": g.note, "subject_type": g.subject_type, "subject_id": g.subject_id, "created_by": g.created_by, "created_at": _iso(g.created_at), "updated_at": _iso(g.updated_at),
...
def seed(dataset: str, now: datetime) -> List[GraphicRow]:
    ...
    def G(i, type_, kind, section, name, geom, wf=None, wt=None, status="active", note="", st=None, sid=None, by="S3 Operations"):
        return GraphicRow(id=f"gfx_{i:03d}", type=type_, kind=kind, section=section, name=name, geometry_json=json.dumps(geom), window_from=wf, window_to=wt, status=status, note=note,
                          subject_type=st, subject_id=sid, created_by=by, created_at=now - timedelta(hours=20), updated_at=now - timedelta(hours=20))
    if dataset == "cab":
        return [
