...
export function readDestination(hash = window.location.hash): Destination {
  ...
  return { page: page === 'work' ? 'workspace' : page === 'workspace' ? page : 'cop', section: sections.includes(section) ? section as SectionCode : 'S2', tab: page === 'work' ? 'analysis' : tab || 'overview', record: new URLSearchParams(query).get('record') }
}
...
export function useDestination() {
  ...
  const navigate = (next: Partial<Destination>) => { const d = readDestination(destinationHash({ ...destination, record: null, ...next })); window.location.hash = destinationHash(d); setDestination(d) }
