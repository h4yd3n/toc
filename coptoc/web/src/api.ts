...
/** §5.11 — a typed proposal about the case graph. It names the finding behind it, so it inherits that citation. */
export interface WorkGraphProposal { finding_index: number; kind: 'entity' | 'relationship' | 'event'; entity_type: string; name: string; from_name: string; to_name: string; link_type: string }
export interface WorkAnalysis { title: string; summary: string; findings: { claim: string; citations: { source_id: string; quote: string }[]; uncertainty: string }[]; gaps: string[]; proposed_tasks: string[]; proposed_graph?: WorkGraphProposal[] }
export interface WorkAssignment { location_id: string | null; subject_type?: 'operation' | 'requirement' | null; subject_id?: string | null; id: string; section: import('./types').SectionCode; case_id: string | null; instruction: string; status: string; owner: string; cadence_minutes: number; created_at: string; last_success_at: string | null; next_at: string }
export interface WorkRun { instruction?: string; attempts?: number; retry_at?: string | null; original?: WorkAnalysis; metrics?: { seconds?: number; usage?: Record<string,number>; repeats?: { finding_index: number; run_id: string; assignment_id: string; basis: string }[] }; id: string; assignment_id: string; status: string; review_status: string; revision: number; result: Partial<WorkAnalysis>; evidence: WorkEvidence[]; provider: string; model: string; error: string; reviewed_by: string; created_at: string; completed_at: string | null; history: { at: string; actor: string; action: string; note: string; result?: WorkAnalysis }[] }
...
export const assignWork = (body: { section: import('./types').SectionCode; case_id?: string; location_id?: string; subject_type?: 'operation' | 'requirement'; subject_id?: string; instruction: string; cadence_minutes: number }) => req<WorkAssignment>('POST', '/v1/work/assignments', body)
...
/** §5.11 — apply one typed proposal into the case graph, as suggested; the analyst still confirms it. */
export const applyWorkProposal = (id: string, index: number) => req<{ kind: string; id: string; status: string; existing: boolean }>('POST', `/v1/work/runs/${id}/graph`, { index })
/** §5.12 — which open case a loose report probably belongs to, and the reason each match gives. */
export const caseMatches = (reportId: string) => req<{ report_id: string; attached_to: string | null; matches: { case_id: string; title: string; kind: string; reasons: { kind: string; detail: string }[] }[] }>('GET', `/v1/s2/reports/${reportId}/case-matches`)
