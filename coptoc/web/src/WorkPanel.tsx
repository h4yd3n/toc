...
import type { Case, Location, Operation, Requirement, Role, SectionCode } from './types'
...
export function WorkPanel({ section, caseId, role, canEdit, reload, act, busy, onOpenCase, selectedRun, locations = [] }: { locations?: Location[]; section?: SectionCode; caseId?: string | null; role: Role; canEdit: (s: SectionCode) => boolean; reload: number; act: Act; busy: string | null; onOpenCase?: (id: string) => void; selectedRun?: string | null }) {
  ...
  const [subject, setSubject] = useState('')   // "operation:<id>" or "requirement:<id>" — §5.12 a scoped assignment
  ...
  useEffect(() => { let alive = true; api.listOperations().then(o => { if (alive) setOps(o) }).catch(() => {}); if (scope === 'S2') api.listRequirements().then(r => { if (alive) setReqs(r) }).catch(() => {}); return () => { alive = false } }, [scope, reload])
  ...
  return <section className="work-panel">
    ...
    {board && <p className="ws-notice">{board.provider.configured ? `Provider: ${board.provider.provider} · ${board.provider.model}` : (role === 'battle_captain' ? 'AI provider is off or incomplete. Configure the staff analysis provider, model, and API key in Settings.' : 'AI provider is off or incomplete. A Battle Captain can configure it in Settings.')} Drafts require review before release.</p>}
    {create && <form className="ws-form" onSubmit={e => { e.preventDefault(); act('assigning analysis', async () => { if(editingAssignment) await api.editWork(editingAssignment,instruction,cadence); else await api.assignWork({ section: scope, case_id: chosenCase || undefined, location_id: locationId || undefined, subject_type: subject ? subject.split(':')[0] as 'operation' | 'requirement' : undefined, subject_id: subject ? subject.split(':')[1] : undefined, instruction, cadence_minutes: cadence }); setCreate(false) }) }}>
      ...
      <label>Narrow to<select disabled={!!editingAssignment} value={subject} onChange={e => setSubject(e.target.value)} title="an operation brings its own tasks and resources; a requirement brings the reporting inside its radius">
        <option value="">Nothing — the section&apos;s own scope</option>
        {ops.map(o => <option key={o.id} value={`operation:${o.id}`}>OP · {o.title}</option>)}
        {scope === 'S2' && reqs.filter(r => r.status === 'active').map(r => <option key={r.id} value={`requirement:${r.id}`}>NAI · {r.subject_name || r.id}</option>)}
      </select></label>
    ...
    {selectedRun?.startsWith('run_') && board && !runs.some(r=>r.id===selectedRun) && <p className="ws-notice">This result is unavailable to your current profile or no longer exists.</p>}
    ...
    {visible.map(r => { const a = assignments.find(a => a.id === r.assignment_id)!; return <article className="ws-card" key={r.id}>
      ...
      {r.error && <p className="ws-error">{r.error}{r.retry_at && ` · Next attempt: ${date(r.retry_at)}`}</p>}
      {open === r.id && <><p className="dim">Assigned by {a.owner} · {r.provider && `${r.provider} / ${r.model} · `}Last successful check: {date(a.last_success_at)}</p>
        <details><summary>Run details · {r.evidence.length} sources</summary><p>{r.instruction || a.instruction}</p><p className="dim">{r.attempts ?? 0} attempts{r.metrics?.seconds !== undefined && ` · ${r.metrics.seconds}s processing`}</p>{r.metrics?.usage && <pre>{JSON.stringify(r.metrics.usage,null,2)}</pre>}</details>
        {a.case_id && onOpenCase && <button className="ws-link" onClick={() => onOpenCase(a.case_id!)}>Open supporting case →</button>}
        {r.status === 'completed' && <ResultEditor caseScoped={!!a.case_id || r.evidence.some(e=>!!e.case_id || e.context_scopes?.some(s=>!!s.case_id))} key={r.id + ':' + r.revision} run={r} editable={canEdit(a.section)} role={role} busy={busy} act={act} />}
        {r.status === 'unchanged' && <p>Evidence is unchanged since the last successful analysis.</p>}
      </>}
    </article> })}
    {assignments.length > 0 && <details className="ws-card"><summary>Manage assignments ({assignments.length})</summary>{assignments.map(a => <div className="ws-assignment" key={a.id}><strong>{a.instruction}</strong><p className="dim">{a.section}{a.subject_type ? ` · ${a.subject_type} ${a.subject_id}` : ''} · {a.status} · {a.cadence_minutes ? `Checks every ${a.cadence_minutes} minutes` : 'One analysis'} · {a.owner}</p>{canEdit(a.section) && a.status !== 'cancelled' && <div className="ws-actions">{<button disabled={!!busy} onClick={()=>{setEditingAssignment(a.id);setScope(a.section);setChosenCase(a.case_id??'');setLocationId(a.location_id??'');setInstruction(a.instruction);setCadence(a.cadence_minutes);setCreate(true)}}>Edit</button>}{['run', a.status === 'paused' ? 'resume' : 'pause', 'cancel'].map(action => <button key={action} disabled={!!busy} onClick={() => act(action + ' assignment', () => api.changeWork(a.id, action as 'run' | 'resume' | 'pause' | 'cancel'))}>{action === 'run' ? 'Run again' : action}</button>)}</div>}</div>)}</details>}
  </section>
}
function ResultEditor({ run, editable, role, busy, act, caseScoped }: { caseScoped: boolean; run: WorkRun; editable: boolean; role: Role; busy: string | null; act: Act }) {
  ...
  const submit = (action: 'save' | 'review' | 'release' | 'reject') => act(action + ' analysis', async () => { await api.reviewWork(run.id, run.revision, action, {...result,gaps:result.gaps.filter(s=>s.trim()),proposed_tasks:result.proposed_tasks.filter(s=>s.trim())}, note); setEdit(false) })
  return <div className="ws-analysis">
    {edit && <label>Title<input value={result.title} onChange={e=>setResult({...result,title:e.target.value})}/></label>}
    ...
    {result.findings.map((f,i) => <div className="ws-finding" key={i}>{(run.metrics?.repeats ?? []).some(x => x.finding_index === i) && <p className="ws-kicker" title={(run.metrics!.repeats!.find(x => x.finding_index === i)!.basis)}>REPEAT · another assignment already made this finding — not a second source</p>}{edit ? <textarea value={f.claim} onChange={e => setResult({ ...result, findings: result.findings.map((x,j) => i === j ? { ...x, claim: e.target.value } : x) })} /> : <p>{f.claim}</p>}{edit ? <label>Uncertainty<input value={f.uncertainty} onChange={e=>setResult({...result,findings:result.findings.map((x,j)=>i===j?{...x,uncertainty:e.target.value}:x)})}/></label> : f.uncertainty && <p className="dim">Uncertainty: {f.uncertainty}</p>}{edit && <button className="ws-link" onClick={()=>setResult({...result,findings:result.findings.filter((_,j)=>i!==j)})}>Remove finding</button>}{f.citations.map((c,j) => <details className="ws-citation" key={j}><summary>{run.evidence.find(e => e.id === c.source_id)?.label ?? c.source_id}</summary><blockquote>{c.quote}</blockquote><details><summary>Read source</summary><p className="ws-source">{run.evidence.find(e => e.id === c.source_id)?.text}</p></details></details>)}</div>)}
    {edit && <><label>Gaps (one per line)<textarea value={result.gaps.join('\n')} onChange={e=>setResult({...result,gaps:e.target.value.split('\n')})}/></label><label>Proposed tasks (one per line)<textarea value={result.proposed_tasks.join('\n')} onChange={e=>setResult({...result,proposed_tasks:e.target.value.split('\n')})}/></label></>}
    {result.gaps.length > 0 && <><h3>Gaps & competing explanations</h3><ul className="ws-bullets">{result.gaps.map((g,i) => <li key={i}>{g}</li>)}</ul></>}
    {(result.proposed_graph?.length ?? 0) > 0 && caseScoped && <><h3>Proposed for the case graph</h3>
      <p className="dim">Each proposal names the finding behind it and carries that finding&apos;s quote. Applying one adds it to the case as <em>suggested</em> — the analyst still confirms or rejects it in the review queue.</p>
      <ul className="ws-bullets">{result.proposed_graph!.map((p, i) => <li key={i}>
        <span className="ws-kicker">{p.kind.toUpperCase()}{p.kind === 'entity' ? ` · ${p.entity_type}` : p.kind === 'relationship' ? ` · ${p.link_type}` : ''}</span>{' '}
        {p.kind === 'relationship' ? `${p.from_name} → ${p.to_name}` : p.name}
        <span className="dim"> · from finding {p.finding_index + 1}</span>
        {editable && ['review', 'released'].includes(run.review_status) && <button className="ws-link" disabled={!!busy || !!applied[i]} onClick={() => act('applying the proposal', async () => { const r = await api.applyWorkProposal(run.id, i); setApplied(s => ({ ...s, [i]: r.existing ? 'evidence added' : 'added as suggested' })) })}>{applied[i] ?? 'Apply to the case'}</button>}
      </li>)}</ul>
      {!(editable && ['review', 'released'].includes(run.review_status)) && <p className="dim">Send this analysis to review before applying anything it proposes.</p>}</>}
    ...
    {run.history.length > 0 && <details><summary>Review history ({run.history.length})</summary>{run.original?.summary && <details><summary>Original model draft</summary><p>{run.original.summary}</p>{run.original.findings.map((f,i)=><p key={i}>{f.claim}</p>)}</details>}{run.history.map((h,i) => <details key={i}><summary>{date(h.at)} · {h.actor} · {h.action}{h.note && ` — ${h.note}`}</summary><p>{h.result?.summary}</p>{h.result?.findings.map((f,j)=><p key={j}>{f.claim}</p>)}</details>)}</details>}
  </div>
}
