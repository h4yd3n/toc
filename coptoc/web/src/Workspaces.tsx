...
export function SectionSummary({ section, snap, onOpen, onSelect }: { section: SectionCode; snap: Snapshot; onOpen: (tab?: string, record?: string) => void; onSelect: (s: Selection) => void }) {
  ...
  return <div className="section-summary">
    ...
    {exceptions.length > 3 && <button className="ws-link" onClick={() => onOpen()}>View all {exceptions.length} exceptions →</button>}
...
export default function Workspaces({ destination:d, navigate, snap, role, enabled, can, act, busy, reload, onSelect, onArea, onIntsum, onOp, siteForm }: Props) {
  ...
  return <main className="workspace" aria-label={d.page === 'work' ? 'My work' : `${STAFF[section]} workspace`}>
    ...
    <div className="workspace-body">
      ...
      {d.page==='work' ? <><WorkPanel locations={snap.locations} key={d.page+':'+section} role={role} canEdit={s=>can(s,'edit')} reload={reload} busy={busy} act={act} selectedRun={d.record} onOpenCase={id=>navigate({page:'workspace',section:'S2',tab:'cases',record:id})} />{enabled.filter(s=>can(s,'edit')).map(s=><details className="ws-card" key={s}><summary>{s} · {STAFF[s]} taskings</summary><TaskingBox section={s} board={snap.taskings} canEdit={can(s,'edit')} busy={busy} act={act} enabled={enabled}/></details>)}<details className="ws-card"><summary>Battle log · recent recorded activity</summary>{snap.log.map(e=><p key={e.id}><time>{new Date(e.at).toLocaleString()}</time> · {e.summary} <span className="dim">— {e.actor}</span></p>)}</details></> : <>
        ...
        {section==='S1' && ['personnel','assignments'].includes(tab) && <>{can('S1','edit')&&<button className="ws-primary" onClick={()=>setAdding(!adding)}>Add person</button>}{adding&&<PersonForm snap={snap} busy={busy} act={act} onDone={()=>setAdding(false)}/>}<div className="ws-toolbar">{search}{can('S1','edit') && <button className="ws-primary" onClick={()=>setImporting(!importing)}>Import roster</button>}</div>{importing && <section className="ws-card"><UploadDrawer section="S1" busy={busy} act={act} onDone={()=>{}}/><ImportDrawer busy={busy} act={act} onDone={()=>setImporting(false)}/></section>}<section className="ws-card">{tab==='assignments'?<TaskOrg teams={snap.teams} people={snap.people.filter(p=>match(p.name+' '+p.team_name))} onSelect={onSelect} sel={null}/>:<PersonnelList key={query} people={snap.people.filter(p=>match(p.name+' '+p.team_name))} canEdit={can('S1','edit')} busy={busy} act={act} onSelect={onSelect}/>}</section></>}
        {((section==='S1' && tab==='accountability') || (section==='S6' && tab==='incidents')) && <section className="ws-card"><h2>Accountability incidents</h2>{snap.incidents.length===0 && <p className="dim">No incidents recorded. Open a site on the COP to start a roll call.</p>}{snap.incidents.map(i=><button className="section-exception" key={i.id} onClick={()=>onSelect({type:'incident',id:i.id})}><strong>{i.title}</strong><span>{i.accounted}/{i.total} accounted · {i.status}</span></button>)}</section>}
        {section==='S3' && tab==='operations' && <>{can('S3','edit')&&<button className="ws-primary" onClick={()=>setAdding(!adding)}>Create activity</button>}{adding&&<EventForm snap={snap} busy={busy} act={act} onDone={()=>setAdding(false)}/>}<div className="ws-toolbar">{search}{can('S3','edit') && <button className="ws-primary" onClick={()=>setImporting(!importing)}>Import schedule</button>}{can('S3','edit') && <button onClick={()=>setSite(!site)}>Add site</button>}</div>{site && <section className="ws-card">{siteForm}</section>}{importing && <section className="ws-card"><UploadDrawer section="S3" busy={busy} act={act} onDone={()=>{}}/><ImportDrawer busy={busy} act={act} onDone={()=>setImporting(false)}/></section>}<section className="ws-card"><h2>Activities & operations</h2>{snap.events.filter(e=>match(e.name)).map(e=><button className="section-exception" key={e.id} onClick={()=>e.operation?onOp(e.operation.id):onSelect({type:'event',id:e.id})}><strong>{e.name}</strong><span>{e.venue_name} · {e.status} · {new Date(e.start_at).toLocaleString()}</span></button>)}</section></>}
        ...
        {section==='S6' && ['systems','contacts'].includes(tab) && <>{can('S6','edit')&&<button className="ws-primary" onClick={()=>setAdding(!adding)}>Add system</button>}{adding&&<StaffRecordForm kind="system" snap={snap} busy={busy} act={act} onDone={()=>setAdding(false)}/>}<div className="ws-toolbar">{search}{can('S6','edit') && <button className="ws-primary" onClick={()=>setImporting(!importing)}>Import communications</button>}</div>{importing && <section className="ws-card"><UploadDrawer section="S6" busy={busy} act={act} onDone={()=>{}}/></section>}<section className="ws-card"><S6Panel canEditOverride={can('S6','edit')} role={role} busy={busy} act={act} view={tab==='systems'?'systems':'pace'} board={{...snap.s6,systems:snap.s6.systems.filter(x=>match(x.name+' '+x.location_name))}}/></section></>}
      </>}
    </div>
  </main>
}
