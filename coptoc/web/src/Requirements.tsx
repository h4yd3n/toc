...
import type { Coverage, Plan, Requirement, Selection, SourceInfo } from './types'
...
export function RequirementsPanel({ reload, busy, act, onSelect, role }: { reload: number; busy: string | null; act: (l: string, f: () => Promise<unknown>) => void; onSelect: (s: Selection) => void; role: string }) {
  ...
  useEffect(() => { api.listRequirements().then(setReqs).catch(() => {}); api.getCoverage().then(setCov).catch(() => {}) }, [reload])
  ...
  return (<>
    <div className="section-label">REQUIREMENTS <span className="dim">{reqs.length} active · {cov ? `${cov.avg_coverage_pct}% avg coverage` : ''}</span>
      ...
      <button className="mini" onClick={() => setShowSources(v => !v)}>SOURCES</button></div>
